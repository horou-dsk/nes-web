...
const KEY_BINDS1: [Button; 8] = [
    Button::Joypad1B, Button::Joypad1A, Button::Start, Button::Select,
    Button::Joypad1Up, Button::Joypad1Left, Button::Joypad1Down, Button::Joypad1Right
];

const KEY_BINDS2: [Button; 8] = [
    Button::Joypad2B, Button::Joypad2A, Button::Start, Button::Select,
    Button::Joypad2Up, Button::Joypad2Left, Button::Joypad2Down, Button::Joypad2Right
];
...
impl WasmNes {
    ...
    pub fn handle_key_state(&mut self, key_state: &[u8]) {
        let key_binds = match key_state[8] {
            1 => &KEY_BINDS1,
            _ => &KEY_BINDS2
        };
        for (index, button) in key_binds.iter().enumerate() {
            if key_state[index] == 1 {
                self.nes.press_button(to_button_internal(button));
            } else {
                self.nes.release_button(to_button_internal(button));
            }
